...
xlabs <- colnames(dc[, -n])
for (i in 1: n){
  boxplot(price_range,dc[ , i], data= dc, ylab= "Price Range" , xlab=xlabs)
}
...
screeplot(comp_tstd, type = "lines")
comp_treduced <- comp_tstd$scores[, 1:5]
glmt <- glm(price_range~.,family = binomial(link = "logit"), data = train)
prd2 <- predict.glm(glmt, newdata=test,type="response")

prd2[prd2 >= 0.5] <- 1;
prd2[prd2 < 0.5] <- 0;
a2 <- ifelse(test$price_range == prd2, 1,0)
sum(a2)/500
